...
from redpost.data.hf_bert_tokenizer import HfBertTokenizer
from redpost.models.redpunc_bert import RedPuncBert
from redpost.data.token_dict import TokenDict

class RedPostConfig:
    def __init__(self, use_gpu=False, sentence_max_length=-1):
...
class RedPost:
    ...
    def from_pretrained(cls, model_dir, config):
        ...
        assert isinstance(config, RedPostConfig)
    ...
    def process_with_timestamp(self, batch_timestamp, batch_uttid=None):
        ...
        for txts in punc_txts:
            new_txts = [(RuleBasedTxtFix.fix(txt[0]), txt[1], txt[2]) for txt in txts]
